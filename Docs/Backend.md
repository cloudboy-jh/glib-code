...
- Session IDs are indexed in app config (`sessions-index.json`) so agent routes resolve by `sessionId`, not client-supplied `projectPath`.
- Session-scoped agent routes do not require `projectPath` for send/stream/abort/delete. Legacy project-path fallback exists only to recover pre-index sessions.

| Method | Path | Description |
|---|---|---|
| `POST` | `/api/agent/sessions` | Create a new agent session and initialize matching GitTrix session metadata. |
| `POST` | `/api/agent/sessions/:id/send` | Persist user turn, broadcast it, and run pi through the configured runtime in the session ephemeral workspace. Body includes `prompt` and optional context/attachments. |
