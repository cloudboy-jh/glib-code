<template>
  <UiDialog dialog-class="max-w-[560px] max-h-[80vh] overflow-y-auto" @close="$emit('close')">
    ...
    <div class="p-6 pt-1 pb-4">
      ...
      <template v-for="group in groupedItems" :key="group.category">
        <div class="mt-4 mb-1 text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">{{ group.label }}</div>
        <div class="space-y-1">
          <button
          ...
          >
            ...
            <span class="min-w-0 flex-1">
              ...
              <span v-if="item.inlineArgs && item.argHint" class="mt-0.5 block text-[11px] text-muted-foreground/60">Usage: /{{ item.value }} {{ item.argHint }}</span>
